...
const traverse$1 = __babel_traverse.default.default || __babel_traverse.default;
function analyzeClassNameExpression(node) {
	if (!node) return {
		type: "static",
		value: ""
	};
	if (__babel_types.isStringLiteral(node)) return {
		type: "static",
		value: node.value
	};
	if (__babel_types.isJSXExpressionContainer(node)) return analyzeClassNameExpression(node.expression);
	if (__babel_types.isTemplateLiteral(node) && node.expressions.length === 0) return {
		type: "static",
...
function analyzeElementDeletionSafety(path$5) {
	...
	while (current) {
		if (__babel_types.isConditionalExpression(current)) return {
			type: "dynamic",
			reason: "conditional-expression"
		};
		if (__babel_types.isCallExpression(current) && __babel_types.isMemberExpression(current.callee) && __babel_types.isIdentifier(current.callee.property) && current.callee.property.name === "map") return {
			type: "dynamic",
			reason: "map-expression"
		};
		if (__babel_types.isLogicalExpression(current) || __babel_types.isSequenceExpression(current) || __babel_types.isArrayExpression(current)) return {
			type: "dynamic",
			reason: "complex-parent"
		};
		if (__babel_types.isFunction(current) || __babel_types.isArrowFunctionExpression(current) || __babel_types.isJSXElement(current)) break;
...
function extractTextContent(children) {
	...
	if (children.some((child) => __babel_types.isJSXElement(child) || __babel_types.isJSXFragment(child))) return {
		type: "dynamic",
		reason: "has-children"
	};
	if (children.every((child) => __babel_types.isJSXText(child) || __babel_types.isJSXExpressionContainer(child) && __babel_types.isStringLiteral(child.expression))) return {
		type: "static",
		value: children.map((child) => {
			if (__babel_types.isJSXText(child)) return child.value;
			else if (__babel_types.isJSXExpressionContainer(child) && __babel_types.isStringLiteral(child.expression)) return child.expression.value;
			return "";
		}).join("").trim()
	};
	return {
	...
	try {
		...
		traverse$1(ast, { JSXElement(path$5) {
			...
			if (loc.start.line === line && Math.abs(loc.start.column - col) <= 5) {
				elementFound = true;
				const classNameAttr = openingElement.attributes.find((attr) => __babel_types.isJSXAttribute(attr) && __babel_types.isJSXIdentifier(attr.name) && attr.name.name === "className");
		...
		traverse(ast, { JSXElement(path$5) {
			...
			if (loc.start.line === line && Math.abs(loc.start.column - col) <= 5) {
				if (updates.className !== void 0) {
					...
					else if (classNameAttr) classNameAttr.value = __babel_types.stringLiteral(updates.className);
					else openingElement.attributes.push(__babel_types.jsxAttribute(__babel_types.jsxIdentifier("className"), __babel_types.stringLiteral(updates.className)));
				}
				if (updates.textContent !== void 0) {
					path$5.node.children = [];
					if (updates.textContent.trim() !== "") path$5.node.children.push(__babel_types.jsxText(updates.textContent));
				}
				modified = true;
				path$5.stop();
			}
				...
				if (importer) {
					...
					if (!await exists(resolvedPath)) {
						try {
							...
							if (debug) console.error("Error creating parent directory", error);
						}
